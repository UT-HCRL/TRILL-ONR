...
def pick_socket(env, lh_target_pos, lh_input_quat, gain=0.02, stage=0):

    if env.socket is None:
        return False
    grasping_state = sim_util.get_grasping_state(env.sim, env.robot, env.socket)
...
def place_socket(env, lh_target_pos, lh_input_quat, gain=0.02, stage=0):
...
def hold_hand(env, cur_target_pos, input_pos, input_quat, grasping, gain=0.02, tolerance=0.01, hand="left"):
    ...
    return new_target_pos, input_quat, True, done
...
def main(gui, env_type, cam_name="upview", subtask=0, save_video=True):
    ...
    while not done:
        ...
        if stage < 2:
            lh_target_pos, lh_input_quat, grasping, task_done = pick_socket(env, lh_target_pos, lh_input_quat, gain=0.02, stage=stage)
            stage += task_done
        ...
        if stage < 6 and stage >=4:
            lh_target_pos, lh_input_quat, grasping, task_done = place_socket(env, lh_target_pos, lh_input_quat, gain=0.02, stage=stage)
            stage += task_done
